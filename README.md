<div align="center">

<img src="./assets/banner.png" alt="Profile banner" width="100%"/>

<br>

# Jaime Alonso Pulido

### Computer Engineering & Business Administration Student  
### AI · Data Science · Automation · Systems

**Building technology that connects intelligent systems with real-world business impact.**

<br>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A192F?style=for-the-badge&logo=linkedin&logoColor=00C2FF)](TU-LINKEDIN)
[![Email](https://img.shields.io/badge/Email-0A192F?style=for-the-badge&logo=gmail&logoColor=00C2FF)](mailto:TU-EMAIL)
[![GitHub](https://img.shields.io/badge/GitHub-0A192F?style=for-the-badge&logo=github&logoColor=00C2FF)](https://github.com/xornic1229)

</div>

---

## About Me

<img align="right" src="./assets/profile-card.png" alt="Profile card" width="260"/>

I am a double-degree student in **Computer Engineering** and **Business Administration** at **Universidad Carlos III de Madrid**.

My profile combines technical training in programming, algorithms and systems with a business-oriented perspective focused on strategy, scalability and real-world execution.

I am especially interested in **Artificial Intelligence**, **Data Science**, **Systems** and **Process Automation**.

Alongside my university studies, I am strengthening my software development skills through **42 Madrid**, with a strong focus on autonomy, problem-solving and project-based learning.

<br clear="right"/>

---

## Current Focus

<div align="center">

<img src="./assets/focus.png" alt="Current focus" width="85%"/>

</div>

- Artificial Intelligence and applied data systems
- Python programming and software engineering fundamentals
- Process automation and data cleaning solutions
- Systems thinking, scalable workflows and technical execution
- Building a hybrid profile between technology and business

---

## Tech Stack

<div align="center">

### Languages

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/python/python-original.svg" alt="Python" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/c/c-original.svg" alt="C" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/javascript/javascript-original.svg" alt="JavaScript" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/mysql/mysql-original.svg" alt="SQL" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/html5/html5-original.svg" alt="HTML5" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/css3/css3-original.svg" alt="CSS3" width="48"/>

<br><br>

### Tools & Platforms

<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/linux/linux-original.svg" alt="Linux" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/git/git-original.svg" alt="Git" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/github/github-original.svg" alt="GitHub" width="48"/>
&nbsp;&nbsp;
<img src="https://raw.githubusercontent.com/devicons/devicon/master/icons/bash/bash-original.svg" alt="Bash" width="48"/>

</div>

---

## Featured Areas

<div align="center">

| Area | What I am building towards | Main tools |
|:---:|---|:---:|
| **Software Engineering** | Strong programming foundations, clean code and systems thinking | `C` `Python` `Linux` |
| **Data Science** | Data cleaning, analysis and structured problem solving | `Python` `SQL` |
| **Automation** | Tools that reduce repetitive work and improve business processes | `Python` `Scripts` |
| **Business & Strategy** | Technology solutions connected to real operational needs | `ADE` `Entrepreneurship` |

</div>

---

## GitHub Stats

<div align="center">

<img width="700" src="https://streak-stats.demolab.com?user=xornic1229&theme=tokyonight&hide_border=true&background=0A192F&ring=00C2FF&fire=00C2FF&currStreakLabel=00C2FF"/>

</div>

---
